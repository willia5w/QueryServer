...
Index BuildMovieIndex(LinkedList movies, enum IndexField field_to_index){
  ...
  Index movie_index = CreateHashtable(NumElementsInLinkedList(movies));
  ...
  if (NumElementsInLinkedList(movies) == 0) {
    exit(EXIT_SUCCESS);  // Exit program if passing empty movie list
  }
  ...
  while (LLIterHasNext(iter)) {
    ...
    if (result == 2) {
      ...
      // AddMovieToIndex will never return anything besides zero. It is
      // structured to get the MovieSet in the case of a collision or 
      // create a new MovieSet if this movie was not already contained.
    }
  }
  return movie_index;
}

...
int KeyInChain(LLIter lliter, uint64_t key, HTKeyValue **keyvalue) {
  Assert007(lliter != NULL);

  while (1) {
    LLIterGetPayload(lliter, (void **) keyvalue);
    if ((*keyvalue)->key == key) {
      return 0;  // Node has key, success
    }
    if (LLIterHasNext(lliter) == 0) {
      break;  // Cannot iterate further, no nodes contain key
    }
    LLIterNext(lliter);
  }
  return -1;  // No nodes contained key, fail
}
...
int LookupInHashtable(Hashtable ht, uint64_t key, HTKeyValue *result) {
  ...
  if (KeyInChain(lliter, key, &temp_payload) == 0) {
    result->key = temp_payload->key;
    result->value = temp_payload->value;
